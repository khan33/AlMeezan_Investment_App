...
@interface PAStorageHelper ()

@property (nonatomic, assign) BOOL traversed;

@end

@implementation PAStorageHelper
...
+ (NSPersistentStoreCoordinator *) persistentStoreCoordinator
{
    ...
    NSArray* modelNames = @[@"AlMeezan"];
    
    if (![PAStorageHelper iterativeMigrateURL:storeURL
...
{
    return [PAStorageHelper isManagedObjectAlreadyExist:entityName withPredicate:filter inContext:[PAStorageHelper managedObjectContext]];
}
...
+ (NSError*)errorWithCode:(NSInteger)code description:(NSString*)description
{
    ...
    return [NSError errorWithDomain:[PAStorageHelper errorDomain]
                               code:code
                           userInfo:userInfo];
...
- (NSDictionary*) toDictionary
{
    ...
    for (NSString* relationship in relationships) {
        ...
        if ([value isKindOfClass:[NSSet class]]) {
            ...
            for (PAStorageHelper* relatedObject in relatedObjects) {
        ...
        else if ([value isKindOfClass:[PAStorageHelper class]]) {
            // To-one relationship
            
            PAStorageHelper* relatedObject = (PAStorageHelper*) value;
...
- (void) populateFromDictionary:(NSDictionary*)dict
{
    ...
    for (NSString* key in dict) {
        ...
        if ([value isKindOfClass:[NSDictionary class]]) {
            ...
            PAStorageHelper* relatedObject =
            [PAStorageHelper createManagedObjectFromDictionary:(NSDictionary*)value
                                                           inContext:context];
            ...
            for (NSDictionary* relatedObjectDict in relatedObjectDictionaries) {
                PAStorageHelper* relatedObject =
                [PAStorageHelper createManagedObjectFromDictionary:relatedObjectDict
                                                               inContext:context];
...
+ (PAStorageHelper*) createManagedObjectFromDictionary:(NSDictionary*)dict
                                                   inContext:(NSManagedObjectContext*)context
{
    NSString* class = [dict objectForKey:@"class"];
    PAStorageHelper* newObject =
    (PAStorageHelper*)[NSEntityDescription insertNewObjectForEntityForName:class
